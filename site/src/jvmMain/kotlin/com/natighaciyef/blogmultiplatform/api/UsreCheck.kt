package com.natighaciyef.blogmultiplatform.api

import com.natighaciyef.blogmultiplatform.data.MongoDB
    ...
    try {
        ...
        val user = userRequest?.let {
            context.data.getValue<MongoDB>()
                .checkUserExistence(user = User(username = it.username, password = hashPassword(it.password)))
        }

        if (user != null) {
            context.res.setBodyText(
                Json
                    .encodeToString(UserWithoutPassword(id = user.id, username = user.username))
            )
        }else{
